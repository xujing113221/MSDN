...
parser.add_argument("--caption_use_dropout", action="store_const", const=0.5, default=0.0, help="Set to use dropout in caption model")
...
def main():
    ...
    optimizer = network.get_optimizer(lr, optimizer_select, args, vgg_features_var, rpn_features, hdn_features, language_features)
    ...
    if args.evaluate:
        ...
        for idx, top_N in enumerate(top_Ns):
            print("[Recall@{top_N:d}] {recall:2.3f}%% (best: {best_recall:2.3f}%%)".format(top_N=top_N, recall=recall[idx] * 100, best_recall=best_recall[idx] * 100))
...
def train(train_loader, target_net, optimizer, epoch):
    ...
    for i, (im_data, im_info, gt_objects, gt_relationships, gt_regions) in enumerate(train_loader):
        ...
        #         w.add_graph(target_net, im_data, True)
        ...
        if args.enable_clip_gradient:
            network.clip_gradient(target_net, 10.0)
        optimizer.step()
        ...
        if (i + 1) % args.log_interval == 0:
            print(
                ...
                "RPN Loss: {rpn_loss.avg: .4f}".format(epoch, i + 1, len(train_loader), batch_time=batch_time, lr=args.lr, loss=train_loss, rpn_loss=train_rpn_loss, solver=args.solver)
            )

            print("\t[Loss]\tobj_cls_loss: %.4f\tobj_box_loss: %.4f" % (train_obj_cls_loss.avg, train_obj_box_loss.avg)),
            ...
            print("\n\t[object]\ttp: %.2f, \ttf: %.2f, \tfg/bg=(%d/%d)" % (accuracy_obj.ture_pos * 100.0, accuracy_obj.true_neg * 100.0, accuracy_obj.foreground, accuracy_obj.background))
            print("\t[predicate]\ttp: %.2f, \ttf: %.2f, \tfg/bg=(%d/%d)" % (accuracy_pred.ture_pos * 100.0, accuracy_pred.true_neg * 100.0, accuracy_pred.foreground, accuracy_pred.background))
            print("\t[region]\ttp: %.2f, \ttf: %.2f, \tfg/bg=(%d/%d)" % (accuracy_reg.ture_pos * 100.0, accuracy_reg.true_neg * 100.0, accuracy_reg.foreground, accuracy_reg.background))
...
def test(test_loader, net, top_Ns):
    ...
    for i, (im_data, im_info, gt_objects, gt_relationships, gt_regions) in enumerate(test_loader):
        # Forward pass
        total_cnt_t, rel_cnt_correct_t = net.evaluate(im_data, im_info, gt_objects.numpy()[0], gt_relationships.numpy()[0], gt_regions.numpy()[0], top_Ns=top_Ns, nms=True)
        rel_cnt += total_cnt_t
        ...
        if (i + 1) % 500 == 0 and i > 0:
            for idx, top_N in enumerate(top_Ns):
                print("[%d/%d][Evaluation] Top-%d Recall: %2.3f%%" % (i + 1, len(test_loader), top_N, rel_cnt_correct[idx] / float(rel_cnt) * 100))
